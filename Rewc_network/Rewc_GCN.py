...
from torch.nn import ModuleList, BatchNorm1d, Conv2d, Dropout
...
class Rewc_GCN(torch.nn.Module):
    def __init__(self,
                 num_features,
                 num_classes,
                 dropout,
        ...
        for i in range(self.num_layers):
            ...
            cg_layer = GCNConv(in_channels=in_channel, out_channels=out_channel)
    ...
    def pooling(self, x, batch):
        if self.global_pool == "max":
            ...
            pass
    def topkpooling(self, x, batch, edge_index):
        self.pool = TopKPooling(self.emb_sizes[0], ratio=0.8).to(self.device)
        if self.topk:
            return self.pool(x, edge_index, None, batch)
        else:
            return x, None, None, batch, None, None
